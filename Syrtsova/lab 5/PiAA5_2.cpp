...
void info() {
	...
	for (int i = 0; i < bohr.size(); i++) {
		...
		else cout << "Вершина-родитель с номером " << bohr[i].par << endl;
...
int main() {
	...
	for (int i = 0; i < num.size(); i++) {
		if (num[i].index < patterns_pos[num[i].pattern_num] - 1) continue;//если индекс подшаблона меньше позиции подшаблона в строке
